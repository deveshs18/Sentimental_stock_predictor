...
def main():
    ...
    try:
        # Load ingested data
        try:
            combined_df = pd.read_csv("data/merged_sentiment_output_with_companies.csv")
        except FileNotFoundError:
            logger.error("merged_sentiment_output_with_companies.csv not found. Please run the data ingestion and entity extraction pipelines first.")
            return
        ...
        sentiment_df.to_csv('data/processed/sentiment_analysis.csv', index=False)
        
        # Add weighted_sentiment to combined_df for company aggregation
        combined_df['weighted_sentiment'] = sentiment_df['weighted_sentiment']
        ...
        if 'company' in combined_df.columns and not combined_df['company'].isna().all():
            try:
                ...
                logger.error(f"Error in company sentiment aggregation: {e}")
                raise
